...
N1 = length(acc_time_data);
  ...
  df = 1 / (N*dt); % differential frequency element
  Nyq = 1 / (2*dt); % Nyquist sampling frequency
  acc_freq_data = fftshift(fft(acc_time_data));%performes dtft on acceleration data and makes it readable
